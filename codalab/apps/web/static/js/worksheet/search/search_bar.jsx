...
function searchQuery(query) {
  ...
  if (query.length > 2) {
    return (dispatch) => {
      ...
      let query_string = query.split(" ").reduce((accumulated_result, cur) => {
        return accumulated_result + encodeURIComponent(`keywords=${cur}`) + "&";
      });
      query_string += `keywords=${encodeURIComponent(".limit=10")}`;

      return fetch(`/rest/worksheets?${query_string}`, {
